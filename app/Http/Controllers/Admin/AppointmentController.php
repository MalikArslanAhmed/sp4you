...
class AppointmentController extends Controller
{
    ...
    public function index()
    {
        ...
        $appointments = Appointment::with(['clients', 'assigned_staffs', 'status', 'media']);
        if (!auth()->user()->is_admin) {
            $appointments = $appointments->whereHas('assigned_staffs', function ($q) {
    ...
    public function update(UpdateAppointmentRequest $request, Appointment $appointment)
    {
        $req_data = $request->all();
        if (!auth()->user()->is_admin) {
            $app  =  $appointment->load('assigned_staffs');
            $ass_staff = $app['assigned_staffs']->toArray();
            $req_data['assigned_staffs'] = array_map(function ($item) {
