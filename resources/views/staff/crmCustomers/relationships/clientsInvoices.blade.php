...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-clientsInvoices">
                <thead>
                    <tr>
                        <th width="10">
                    ...
                    @foreach($invoices as $key => $invoice)
                        <tr data-entry-id="{{ $invoice->id }}">
                            ...
                            <td>
                                ...
                                {{-- @can('invoice_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('staff.invoices.edit', $invoice->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan --}}

                                @can('invoice_delete')
                                    <form action="{{ route('staff.invoices.destroy', $invoice->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-clientsInvoices:not(.ajaxTable)').DataTable({ buttons: dtButtons })
