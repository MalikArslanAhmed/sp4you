@can('crm_document_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('staff.crm-documents.create') }}">
                        ...
                        <tr data-entry-id="{{ $crmDocument->id }}">
                            ...
                            <td>
                                @can('crm_document_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('staff.crm-documents.show', $crmDocument->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('crm_document_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('staff.crm-documents.edit', $crmDocument->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('crm_document_delete')
                                    <form action="{{ route('staff.crm-documents.destroy', $crmDocument->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('crm_document_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('staff.crm-documents.massDestroy') }}",
