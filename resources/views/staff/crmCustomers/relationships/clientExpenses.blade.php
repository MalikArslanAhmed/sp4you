@can('expense_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('staff.expenses.create') }}">
                ...
                <tbody>
                    @foreach($expensesDetails as $key => $eDetail)
                        <tr data-entry-id="{{ $eDetail->expense->id }}">
                            ...
                            <td>
                                @can('expense_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('staff.expenses.show', $eDetail->expense->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                {{-- @can('expense_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('staff.expenses.edit', $eDetail->expense->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan --}}

                                @can('expense_delete')
                                    <form action="{{ route('staff.expenses.destroy', $eDetail->expense->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('expense_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('staff.expenses.massDestroy') }}",
