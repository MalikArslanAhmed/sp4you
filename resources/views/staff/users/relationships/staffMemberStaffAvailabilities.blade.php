...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-staffMemberStaffAvailabilities">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $staffAvailability->id }}">
                            ...
                            <td>
                                @can('staff_availability_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.staff-availabilities.show', $staffAvailability->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('staff_availability_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.staff-availabilities.edit', $staffAvailability->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('staff_availability_delete')
                                    <form action="{{ route('admin.staff-availabilities.destroy', $staffAvailability->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-staffMemberStaffAvailabilities:not(.ajaxTable)').DataTable({ buttons: dtButtons })
